...
const clientValidationRules = () => {
  ...
  return [
    ...
    body('client_id').if(body('protocol').custom((value)=>{return value==='oidc'})).optional({checkFalsy:true}).isString().isLength({min:4, max:36}),
    ...
    body('policy_uri').isString().exists().custom((value)=> value.match(reg.regSimpleUrl)).withMessage('Invalid policy Uri value!'),
    ...
    body('entity_id').if(body('protocol').custom((value)=>{return value==='saml'})).optional({checkFalsy:true}).isString().isLength({min:4, max:256}).custom((value)=> value.match(reg.regSimpleUrl)).withMessage('Invalid entity_id uri value!'),
