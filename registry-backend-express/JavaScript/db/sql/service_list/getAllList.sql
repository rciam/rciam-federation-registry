...
SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,owned,status,type,state,group_manager,notification,deleted,comment,group_id
  FROM
  (SELECT service_id,id AS petition_id,comment,service_description,logo_uri,service_name,integration_environment,CASE WHEN service_petition_details.comment IS NOT NULL THEN true ELSE false END AS notification,CASE WHEN requester=${sub} THEN true ELSE false END AS owned,status,type,null AS state,false AS deleted,group_id
  FROM service_petition_details WHERE reviewed_at IS NULL AND type='create') as petitions LEFT JOIN group_subs USING (group_id)
