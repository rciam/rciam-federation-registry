SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,CASE WHEN owned IS NULL THEN false ELSE owned END,status,type,state,group_manager,CASE WHEN notification IS NULL THEN false ELSE notification END AS notification,deleted,comment
FROM
  ...
  (SELECT id AS group_id,true AS owned,is_owner AS group_manager FROM groups LEFT JOIN group_subs ON groups.id=group_subs.group_id WHERE sub=${sub}) AS group_ids
  USING (group_id)
...
UNION
SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,owned,status,type,state,group_manager,notification,deleted,comment
  FROM
