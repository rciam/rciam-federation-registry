...
FROM
  (SELECT id AS service_id,service_description,logo_uri,service_name,requester,integration_environment,group_id FROM service_details WHERE tenant=${tenant_name}) AS service_details LEFT JOIN service_state ON service_details.service_id=service_state.id LEFT JOIN
