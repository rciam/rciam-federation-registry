SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,status,type,state,CASE WHEN group_manager IS NULL then false ELSE group_manager END,deleted,CASE WHEN owned IS NULL THEN false ELSE owned END,CASE WHEN notification IS NULL THEN false ELSE notification END AS notification,comment,group_id,metadata_url,client_id
FROM
  ...
  LEFT JOIN service_details_oidc ON service_details.service_id=service_details_oidc.id LEFT JOIN service_details_saml ON service_details.service_id=service_details_saml.id WHERE deleted=false OR (deleted=TRUE AND state!='deployed')
UNION
SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,status,type,state,CASE WHEN group_manager IS NULL THEN false ELSE group_manager END,deleted,owned,notification,comment,petitions.group_id,metadata_url,client_id
  FROM
