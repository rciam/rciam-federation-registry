...
SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,status,type,state,CASE WHEN group_manager IS NULL THEN false ELSE group_manager END,owned,notification,comment,petitions.group_id,metadata_url,client_id,deployment_type
  FROM
  (SELECT service_id,id AS petition_id,service_description,logo_uri,comment,service_name,integration_environment,CASE WHEN service_petition_details.comment IS NOT NULL THEN true ELSE false END AS notification,CASE WHEN requester=${sub} THEN true ELSE false END AS owned,status,type,null AS state, group_id,null AS deployment_type
