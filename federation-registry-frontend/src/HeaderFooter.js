  ...
  return (
    <div className="header">
      {bannerAlertInfo[0] && (
        <div
          id="noty-info-bar"
          className={"noty-top-" + bannerAlertInfo[0].type + " noty-top-global"}
        >
          <div>{parse(bannerAlertInfo[0].alert_message)}</div>
          <button
            className="noty-top-close link-button"
            onClick={() => {
      ...
      <div className={"tenant_logo_container"}>
        ...
        <h1 className="text-center">{tenant?.config.home_page_title}</h1>
      </div>
    </div>
  );
};
...
export const NavbarTop = (props) => {
  ...
  return (
    <React.Fragment>
      {tenant && (
        <React.Fragment>
          {tenant.config.ribon.active && (
            <div class="corner-ribbon red">{tenant.config.ribon.text}</div>
          )}
              ...
              {user ? (
                <DropdownButton
                  ...
                  title={
                    <React.Fragment>
                      <span
                        style={tenant && { color: tenant?.config?.theme_color }}
                      >
                        {user
                          ? user[tenant.config.claims.display_name_claim]
                          : "login"}
                ...
                >
                  ...
                  <Dropdown.Item
                    onClick={() => {
                      history.push("/" + tenant?.name + "/userinfo");
                    }}
                  >
                    {t("nav_link_userinfo")}
                  </Dropdown.Item>
                  <Dropdown.Item
                    onClick={() => {
                      if (cookies?.federation_logoutkey) {
                        window.location.assign(
                          `${tenant.logout_uri}&id_token_hint=${cookies.federation_logoutkey}`,
                        );
                      } else {
                        window.location.assign(tenant.base_url);
                      }
                    }}
                  >
                    {t("logout")}
                ...
                <React.Fragment>
                  <a
                    href={
                      config.host[tenant?.name] +
                      "tenants/" +
                      tenant?.name +
                      "/login"
                    }
                  >
                    <Button className="log-button" variant="outline-primary">
                      {t("login")}
                    </Button>
                  </a>
                </React.Fragment>
        ...
        <Row className="row justify-content-center">
          ...
          <Col sm="3" className="ssp-footer__item">
            <div className="footer-logo-container">
              {tenant?.config && (
                <a
                  href={tenant?.config?.footer_logo_link ?? "https://grnet.gr/"}
                >
                  <Image
                    className="ssp-footer__item__logo"
                    src={
                      tenant.config?.footer_logo_url ||
                      process.env.PUBLIC_URL + "/grnet.png"
                    }
                    alt="logo"
                  />
                </a>
          ...
          <Col sm="3" className="ssp-footer__item">
            <div className="footer_link_container">
              ...
              <div className="ssp-footer__item__powered">
                <a href={tenant && tenant?.config?.documentation}>
                  Documentation
                </a>
              </div>
            </div>
