...
(
  SELECT email,name,service_name,type,service_id,integration_environment,user_info.tenant FROM
  (
  ...
  UNION
  SELECT email,name,service_details.service_name,type,service_id,petition.integration_environment,user_info.tenant FROM
  (
...
WHERE email IS NOT NULL AND tenant=${tenant};
