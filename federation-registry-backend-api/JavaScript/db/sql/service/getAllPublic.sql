SELECT json_build_object('id',sd.id,'service_name', sd.service_name,'service_description',sd.service_description,
						 ...
						 'country',sd.country,'website_url',sd.website_url,'tenant',sd.tenant,
    ...
    FROM (SELECT *
	FROM ((SELECT id FROM service_state WHERE state!='deleted') AS bar LEFT JOIN service_details USING (id)) AS foo WHERE tenant=${tenant}) as sd
