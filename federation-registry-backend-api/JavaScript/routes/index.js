...
router.get("/tenants/:tenant/organizations", authenticate, (req, res, next) => {
  try {
    db.organizations
      .get(req.query.search_string, req.query.ror)
      .then((organizations) => {
        if (organizations) {
...
router.put(
  "/tenants/:tenant/services/validate",
  adminAuth,
  getData,
  serviceValidationRules({
    optional: true,
  ...
  (req, res, next) => {
    try {
      ...
      db.service_state
        .updateOutdated(outdated_ids)
        .then((result) => {
          res
            .status(200)
            .send(
              "Success, " +
                result.services_turned_outdated +
                " Services where flagged as outdated and " +
                result.services_turned_up_to_date +
                " Services where unflagged.",
            );
        })
        .catch((err) => {
          next(err);
...
router.get(
  ...
  getServicesValidation(),
  validate,
  authenticate_allow_unauthorised,
  (req, res, next) => {
    try {
      if (req.query.tags) {
      ...
      let authorised =
        !!(
          req.user &&
          req.user.role &&
          req.user.role.actions.includes("get_services")
        ) || req.header("X-Api-Key") === process.env.ADMIN_AUTH_KEY;
      if (req.query.outdated === "true") {
        db.service_state
          .getOutdatedServices(req.params.tenant)
          .then(async (outdated_services) => {
        ...
        db.service
          .getAll(req.params.tenant, req.query, authorised)
          .then((result) => {
                  ...
                  if (services) {
                    ...
                    services.forEach((service, index) => {
                      service_state.push({
                        id: service.id,
                        state: "deployed",
                        outdated: service.outdated ? true : false,
                      });
                      ...
                      if (service.protocol === "oidc") {
                        ...
                        if (
                          ...
                          service.post_logout_redirect_uris.length > 0
                        ) {
                          service.post_logout_redirect_uris.forEach(
                            (post_logout_redirect_uri) => {
                              post_logout_redirect_uris.push({
                                owner_id: service.id,
                      ...
                      if (service.protocol === "saml") {
                        if (
                          service.requested_attributes &&
                          service.requested_attributes.length > 0
                        ) {
                    ...
                    queries.push(
                      t.service_details_protocol.addMultiple(services),
                    );
                    ...
                    if (grant_types.length > 0) {
                      queries.push(
                        t.service_multi_valued.addMultiple(
                          grant_types,
                          "service_oidc_grant_types",
                        ),
                      );
                    }
                    if (scopes.length > 0) {
                    ...
                    if (post_logout_redirect_uris.length > 0) {
                      queries.push(
                        t.service_multi_valued.addMultiple(
                          ...
                          "service_oidc_post_logout_redirect_uris",
                        ),
                      );
                    }
                    if (
                      ...
                      queries.push(
                        t.service_multi_valued.addSamlAttributesMultiple(
                          requested_attributes,
                          "service_saml_attributes",
                        ),
                      );
                    }
                    await t
      ...
      .then((tenant) => {
        if (tenant) {
          ...
          if (tenant_config[req.params.tenant].restricted_env) {
            tenant.restricted_environments =
              tenant_config[req.params.tenant].restricted_env;
          }
          tenant.form_config.requested_attributes = requested_attributes.filter(
            (x) =>
              tenant_config[
                req.params.tenant
              ].form.supported_attributes.includes(x.friendly_name),
          );
...
router.get("/tenants/:tenant/login", (req, res) => {
  ...
  if (clients[req.params.tenant]) {
    res.redirect(
      clients[req.params.tenant].authorizationUrl({
    ...
    res.redirect(
      tenant_config[Object.keys(tenant_config)[0]].base_url.split(
        "/" + Object.keys(tenant_config)[0],
      )[0] + "/404",
    );
  }
});
...
// Callback Route
router.get("/callback/:tenant", async (req, res, next) => {
  try {
    const clients = req.app.get("clients");
    const client = clients[req.params.tenant];
    const response = await client.callback(
      process.env.REDIRECT_URI + req.params.tenant,
      req.query,
    );

    const code = await db.tokens.addToken(
    ...
    res.redirect(
      tenant_config[req.params.tenant].base_url + "/code/" + code.code,
    );
  } catch (err) {
    next(err);
  }
    ...
    db.task("deploymentTasks", async (t) => {
      await t.tokens
        ...
        .then(async (response) => {
          if (res) {
            await t.tokens
              .deleteToken(req.params.code)
              .then((deleted) => {
                if (deleted) {
                  let hash = req.app.get("hash");
                  let federation_authtoken_encrypted = CryptoJS.AES.encrypt(
                    response.token,
                    hash,
                  ).toString();
                  try {
                    res.cookie(
                      "federation_authtoken",
                      federation_authtoken_encrypted,
                      {
                        path: "/",
                        domain: req.headers.host.replace(/:[0-9]{0,4}.*/, ""),
                        sameSite: "Strict",
                        secure: true,
                        httpOnly: true,
                      },
                    );
                    res.cookie("federation_logoutkey", response.id_token, {
                      path: "/",
                      domain: req.headers.host.replace(/:[0-9]{0,4}.*/, ""),
                      sameSite: "Strict",
                      secure: true,
...
router.get(
  "/tenants/:tenant/services/:id/error",
  authenticate,
  (req, res, next) => {
    try {
...
router.put(
  "/tenants/:tenant/services/:id/deployment",
  authenticate,
  (req, res, next) => {
    try {
...
router.get("/tenants/:tenant/user", authenticate, (req, res, next) => {
  try {
    var clients = req.app.get("clients");
    let federation_authtoken =
      req.cookies.federation_authtoken ||
      req.headers.authorization.split(" ")[1];
    clients[req.params.tenant]
      .userinfo(federation_authtoken) // => Promise
                        ...
                        .then(async (ticket_data) => {
                          ...
                          if (errors.length > 0) {
                            await t.user
                              ...
                              .then((users) => {
                                let error_services = format_error_email_data(
                                  data,
                                  errors,
                                  users,
                                );
                                  ...
                                  sendMail(
                                    {
                                      ...
                                      url:
                                        "/services/" +
                                        error_data.service_data.service_id,
                                      ...error_data.service_data,
                                    },
                                    "deployment-error-admin-notif.hbs",
                                    error_data.admin_data,
                                  );
                                });
                              })
                              .catch((error) => {
                              ...
                              sendMail(
                                {
                                  ...
                                  state: email_data.state,
                                  tenant: email_data.tenant,
                                  deployment_type: email_data.deployment_type,
                                },
                                "deployment-notification.hbs",
                                [
                                  {
          ...
          if (result) {
            await t.deployer_agents.getAll().then(async (agents) => {
              if (agents) {
                ...
                await t.deployment_tasks
                  .setDeploymentTasks(service_pending_agents)
                  .then((success) => {
                    if (success) {
                      res.status(200).end();
                    } else {
...
router.get(
  ...
  (req, res, next) => {
    try {
      if (
        req.user.role.actions.includes("get_services") &&
        req.user.role.actions.includes("get_petitions")
      ) {
        db.service_list
          .get(req)
          .then((response) => {
        ...
        req.user.role.actions.includes("get_own_services") &&
        req.user.role.actions.includes("get_own_petitions")
      ) {
                ...
                .then(async (service) => {
                  if (service) {
                    await t.service_state
                      ...
                      .then(async (service_state) => {
                        await t.service_errors
                          .getErrorByServiceId(req.params.id)
                          .then((service_error) => {
...
router.get(
  "/tenants/:tenant/services/:id/petitions",
  authenticate,
  (req, res) => {
    try {
      return db.tx("get-history-for-petition", async (t) => {
        if (req.user.role.actions.includes("get_petitions")) {
          await t.service_petition_details
            .getHistory(req.params.id, req.params.tenant)
            .then((petition_list) => {
      ...
      return db.tx("delete-petition", async (t) => {
        try {
          await t.service_petition_details
            ...
            .then(async (belongs) => {
              if (belongs) {
                const deleted = await t.service_petition_details.deletePetition(
                  req.params.id,
                );
                if (deleted) {
      ...
      return db.task("update-petition", async (t) => {
        try {
          if (req.body.type === "delete") {
            await t.service
              ...
              .then(async (service) => {
                if (service) {
                  ...
                  service.type = "delete";
                  await t.petition
                    .update(service, req.params.id, req.params.tenant, req)
                    .then((resp) => {
                      if (resp.success) {
                        res.status(200).json();
                      } else {
            ...
            await t.petition
              .update(req.body, req.params.id, req.params.tenant, req)
              .then((response) => {
                if (response.success) {
                  res.status(200).end();
                } else {
...
// Review Endpoint
router.put(
  "/tenants/:tenant/petitions/:id/review",
  authenticate,
  canReview,
  (req, res, next) => {
    try {
  ...
  db.tx("get-history-for-petition", async (t) => {
    try {
      await isAvailable(
        ...
        req.query.value,
        req.query.protocol,
        req.query.petition_id &&
          typeof parseInt(req.query.petition_id) === "number"
          ? req.query.petition_id
          : 0,
        req.query.service_id &&
          typeof parseInt(req.query.service_id) === "number"
          ? req.query.service_id
          : 0,
        req.params.tenant,
...
// Get group members
router.get(
  "/tenants/:tenant/groups/:group_id/members",
  authenticate,
  view_group,
  (req, res, next) => {
    try {
      db.group
        .getMembers(req.params.group_id, req.params.tenant)
        .then((group_members) => {
          if (group_members) {
...
router.delete(
  ...
  (req, res, next) => {
    try {
      db.group
        .deleteSub(req.params.sub, req.params.group_id)
        .then((response) => {
          if (response) {
            res.status(200).end();
          } else {
...
router.post(
  "/tenants/:tenant/groups/:group_id/invitations",
  authenticate,
  postInvitationValidation(),
  validate,
  canInvite,
...
// Delete invitation
router.delete(
  "/tenants/:tenant/groups/:group_id/invitations/:id",
  authenticate,
  canInvite,
  (req, res, next) => {
    try {
      db.invitation
        .delete(req.params.id, req.params.tenant)
        .then((response) => {
          if (response) {
            res.status(200).end();
          } else {
...
router.put(
  "/tenants/:tenant/invitations/:invite_id/:action",
  authenticate,
  (req, res, next) => {
    try {
      if (req.params.action === "accept") {
        db.tx("accept-invite", async (t) => {
          await t.invitation
            .getOne(req.params.invite_id, req.user.sub, req.params.tenant)
            .then(async (invitation_data) => {
              if (invitation_data) {
        ...
        db.invitation
          .reject(req.params.invite_id, req.user.sub, req.params.tenant)
          .then((response) => {
            if (response) {
              res.status(200).end();
            } else {
...
router.get("/tenants/:tenant/invitations", authenticate, (req, res, next) => {
  try {
    db.invitation
      .getAll(req.user.sub, req.params.tenant)
      .then((response) => {
        if (response) {
...
router.get(
  ...
  (req, res, next) => {
    try {
      db.invitation
        .getByGroupId(req.params.group_id, req.params.tenant)
        .then((invitations) => {
          if (invitations) {
...
// Activate invitation
router.put(
  "/tenants/:tenant/invitations/activate_by_code",
  authenticate,
  (req, res, next) => {
    try {
      db.invitation
        .setUser(req.body.code, req.user.sub, req.params.tenant)
        .then((result) => {
          if (result.success) {
...
router.put(
  ...
  (req, res, next) => {
    try {
      db.deployer_agents
        .update(req.body, req.params.id, req.params.tenant)
        .then((result) => {
          if (result) {
            res.status(200).end();
          } else {
...
router.post(
  ...
  (req, res, next) => {
    try {
      db.deployer_agents
        .add(req.body.agents, req.params.tenant)
        .then(async (result) => {
          if (result) {
            res.status(200).end();
          } else {
...
function is_group_manager(req, res, next) {
  try {
    ...
    if (
      req.user.role.actions.includes("invite_to_group") ||
      req.user.sub === req.params.sub
    ) {
      next();
    } else {
      db.group
        .isGroupManager(req.user.sub, req.body.group_id)
        .then((result) => {
          if (result) {
            next();
          } else {
...
function view_group(req, res, next) {
  try {
    if (req.user.role.actions.includes("view_groups")) {
      ...
      db.group
        .isGroupMember(req.user.sub, req.params.group_id)
        .then((result) => {
          if (result) {
            next();
          } else {
...
function authenticate_allow_unauthorised(req, res, next) {
  try {
    ...
    if (req.headers.authorization) {
      ...
      clients[req.params.tenant]
        .userinfo(TokenArray[1])
        .then((userinfo) => {
          req.user = userinfo;
          if (req.user.sub) {
            db.user_role
              .getRoleActions(req.user.sub, req.params.tenant)
              .then((role) => {
                if (role) {
                  req.user.role = role;
                  next();
...
function canReview(req, res, next) {
  db.service_petition_details
    .getEnvironment(req.params.id, req.params.tenant)
    .then(async (environment) => {
      // Ckecking if review action is restricted for
      if (environment) {
        if (
          req.body.type === "approve" &&
          tenant_config[req.params.tenant].restricted_env.includes(
            environment,
          ) &&
          !req.user.role.actions.includes("review_restricted")
        ) {
          res.status(401).json({ error: "Requested action not authorised" });
        } else if (
          req.user.role.actions.includes("review_petition") ||
          req.user.role.actions.includes("review_restricted")
        ) {
          next();
        } else {
          await db.petition
            .canReviewOwn(req.params.id, req.user.sub)
            .then((service) => {
              if (
                service &&
                tenant_config[req.params.tenant].test_env.includes(environment)
              ) {
                next();
              } else if (service) {
                if (req.user.role.actions.includes("review_own_petition")) {
                  ...
                  res
                    .status(401)
                    .json({ error: "Requested action not authorised" });
                }
              } else {
                res
                  .status(401)
    ...
    return t.user.getUser(userinfo.sub, tenant).then(async (user) => {
      if (user) {
        if (
          ...
          typeof user.eduperson_entitlement !== "object"
        ) {
          user.eduperson_entitlement = [];
        }
        if (
          !userinfo.eduperson_entitlement ||
          typeof userinfo.eduperson_entitlement !== "object"
        ) {
          userinfo.eduperson_entitlement = [];
        }
        await t.user_role
          .getRole(userinfo.eduperson_entitlement, tenant)
          .then(async (role) => {
            if (role) {
              ...
              let queries = [];
              add_entitlements = userinfo.eduperson_entitlement.filter(
                (x) => !user.eduperson_entitlement.includes(x),
              );
              dlt_entitlements = user.eduperson_entitlement.filter(
                (x) => !userinfo.eduperson_entitlement.includes(x),
              );
              ...
              if (add_entitlements.length > 0) {
                queries.push(
                  t.user_edu_person_entitlement.add(add_entitlements, user.id),
                );
              }
              if (dlt_entitlements.length > 0) {
                queries.push(
                  t.user_edu_person_entitlement.dlt_values(
                    dlt_entitlements,
                    user.id,
                  ),
                );
          ...
          .then(async (role) => {
            if (role) {
              ...
              return t.user_info.add(userinfo, tenant).then(async (result) => {
                if (result) {
                  return t.user_edu_person_entitlement.add(
                    userinfo.eduperson_entitlement,
                    result.id,
                  );
                }
...
const generatePreferredUsername = (userinfo, tenant) => {
  ...
  if (
    ...
    userinfo.preferred_username = (
      userinfo.given_name.replace(/[^a-zA-Z0-9]/g, "_").charAt(0) +
      userinfo.family_name.replace(/[^a-zA-Z0-9]/g, "_")
    ).toLowerCase();
  }
  return userinfo;
};
    ...
    return db.tx("user-check", async (t) => {
      ...
      if (req.route.methods.post) {
        if (req.body.type === "create") {
          ...
          await t.service_petition_details
            .openPetition(req.body.service_id, req.params.tenant)
            .then(async (open_petition_id) => {
              if (!open_petition_id) {
                await t.service_details
                  .getProtocol(
                    req.body.service_id,
            ...
            if (petition && petition.status !== "request_review") {
              if (req.body.type === "delete") {
                ...
                if (petition.type === "create" && req.body.type !== "create") {
                  ...
                  return res
                    .status(403)
                    .send({ error: "Tried to edit registration type" });
                }
                ...
                await isAvailable
                  ...
                  .then(async (available) => {
                    if (available) {
                      ...
                      res
                        .status(422)
                        .send({ error: "Protocol id is not available" });
                    }
                  });
              }
            } else {
